...
# Normalize the data to ensure the maximum value is 0.5
max_positive_temp = np.nanmax(positive_temp_values)
max_negative_temp = np.nanmax(np.abs(negative_temp_values))

# Scale the values
if max_positive_temp > 0:
    positive_temp_values = (positive_temp_values / max_positive_temp) * 0.5

if max_negative_temp > 0:
    negative_temp_values = (negative_temp_values / max_negative_temp) * 0.5
...
plt.ylabel('Latitude (degrees)')
plt.xlabel('Flipped Normalized Temperature Value')
