...
input_directory = r'C:\Users\llccl\OneDrive\Documents\To Do\Academics\2nd Year - 1st Semester\Master Thesis\2 - Thesis Proper\5 - Processing\7. LC_LST_Results\1. Compiled_Mosaic'
output_directory = r'C:\Users\llccl\OneDrive\Documents\To Do\Academics\2nd Year - 1st Semester\Master Thesis\2 - Thesis Proper\5 - Processing\7. LC_LST_Results\2. Compiled_Difference'
...
divide_factor = 1  # Define the divide factor
...
def subtract_and_divide_rasters(file1, file2, output_file, divide_factor):
    with rasterio.open(file1) as src1, rasterio.open(file2) as src2:
        ...
        # Handle NaN values in the output (set NaN as nodata for output)
        diff_data = np.where(np.isnan(diff_data), np.nan, diff_data)
        
        # Update the profile to use NaN as nodata
        profile = src1.profile
        profile.update(dtype=rasterio.float32, nodata=np.nan)
        
        # Write the result to a new file, keeping CRS and transform
