...
# Regex pattern to match filenames and extract {prefix} and {suffix}
pattern = re.compile(r"^(.*)_Global_2000-2005_1000m_vat45_LSTday_(.*)-0000000000-(0000032768|0000000000)\.tif$")

# Group files by their {prefix} and {suffix}
grouped_files = defaultdict(list)
for file in os.listdir(input_dir):
    match = pattern.match(file)
    if match:
        prefix, suffix, _ = match.groups()
        grouped_files[(prefix, suffix)].append(os.path.join(input_dir, file))

# Process each group of matching files
for (prefix, suffix), file_list in grouped_files.items():
    if len(file_list) > 1:  # Only process if there are multiple files in the group
        output_name = f"{prefix}_Global_2000-2005_1000m_vat45_LSTday_{suffix}_mosaic.tif"
        print(f"Processing group: {prefix}_Global_2000-2005_1000m_vat45_LSTday_{suffix} ({len(file_list)} files)")
        mosaic_tif_files(file_list, output_name)
        print(f"Mosaicing completed: {output_name}")
    else:
        print(f"Skipping group with insufficient files: {prefix}_Global_2000-2005_1000m_vat45_LSTday_{suffix}")
